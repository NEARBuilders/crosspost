...
export const ProfileView = ({ profile }) => (
  <div className="mb-4 bg-white p-4">
    <div
      ...
      style={{
        backgroundImage: profile.backgroundImage ? `url(${getImageUrl(profile.backgroundImage)})` : undefined
      }}
    ></div>
    <div className="-mt-12 ml-4 flex items-center">
        <Avatar url={getImageUrl(profile.image)} alt={profile.name || "Profile"} size={128} />
    </div>
    <div className="mt-4">
      <h2 className="text-purple text-2xl font-bold">{profile.name || "Anonymous"}</h2>
