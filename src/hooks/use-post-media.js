    ...
    async (index, file) => {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "Unsupported file type. Twitter supports JPEG, PNG, GIF, WebP images and MP4 videos",
          );
        }
        ...
        const existingVideo = currentPosts.some(
          (p) => p.mediaId && p.mediaPreview?.includes("video/mp4"),
        );
        const existingGif = currentPosts.some(
          (p) => p.mediaId && p.mediaPreview?.includes("image/gif"),
        );
        const existingImages = currentPosts.filter(
          (p) =>
            p.mediaId &&
            !p.mediaPreview?.includes("video/") &&
            !p.mediaPreview?.includes("image/gif"),
        ).length;
